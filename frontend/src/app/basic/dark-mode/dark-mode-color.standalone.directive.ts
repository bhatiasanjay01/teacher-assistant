...
@Directive({
  standalone: true,
  selector: '[appDarkModeColorStandalone]',
})
export class DarkModeColorStandaloneDirective {
  ...
  constructor(private el: ElementRef, private themeService: ThemeService) {
    ...
    this._subscription = this.themeService.onThemeChange$().subscribe((theme) => {
      if (theme === ThemeType.default) {
        this.el.nativeElement.style.color = this._defaultColor;
        this.el.nativeElement.style.backgroundColor = this._defaultColorBackground;
