...
export async function fetchStreamedChatContent(options, onResponse: any = null, onFinish: any = null, onError: any = null) {
  try {
    await fetchStreamedChat(options, (responseChunk) => {
      const content = JSON.parse(responseChunk).choices[0].delta.content;
      const functionCall = JSON.parse(responseChunk).choices[0].delta.function_call?.arguments;
      if (content && onResponse) {
        onResponse(content);
      }

      if (functionCall) {
        onResponse(functionCall);
      }
    });
